...
function expr (dsl, doc) {
  ...
  for(var field in dsl) {
    result[field] = expr.eval(dsl[field], doc, field);
  }
...
expr.eval = function (dsl, doc, field) {
...
expr.ops.$not = function (operand, doc) {
  var value = expr.eval(operand, doc);
  return !value;
}
...
expr.ops.$allElementsTrue = function (operands, doc) {
  return expr.ops.$and(operands, doc);
}

expr.ops.$anyElementTrue = function (operands, doc) {
  return expr.ops.$or(operands, doc);
}
...
expr.ops.$setIsSubset = function (operands, doc) {
  ...
  return _.isEqual(common, _.uniq(values[1]));
}
...
expr.ops.$let = function (operand, doc) {
  ...
  for(var field in operand.vars) {
    scope['$'+field] = expr.eval(operand.vars[field], scope);
  }
...
expr.ops.$map = function (operand, doc) {
  var array = expr.eval(operand.input, doc);
  return array.map(function (item) {
    var scope = _.cloneDeep(doc);
    scope['$'+operand.as] = item;
    return expr.eval(operand.in, scope);
  })
}

expr.ops.$literal = function (operand, doc) {
  return operand;
}
...
expr.ops.$dayOfYear = function (operand, doc) {
  var value = expr.eval(operand, doc);
  var start = new Date(value.getFullYear(), 0, 1).getTime();
...
expr.ops.$week = function (operand, doc) {
  var value = expr.eval(operand, doc);
  var start = new Date(value.getFullYear(), 0, 1 - value.getDay());
