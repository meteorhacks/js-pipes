...
function expr (dsl, doc) {
  return expr.evalObject(dsl, doc);
}


expr.evalExpr = function (dsl, doc, field) {
...
expr.evalArray = function (dsls, doc) {
  return dsls.map(function (dsl) {
    return expr.evalExpr(dsl, doc);
  });
};


expr.evalObject = function (dsl, doc) {
  var result = {};

  for(var field in dsl) {
    result[field] = expr.evalExpr(dsl[field], doc, field);
  }
...
expr.ops.$not = function (operand, doc) {
  var value = expr.evalExpr(operand, doc);
  return !value;
};
...
expr.ops.$let = function (operand, doc) {
  ...
  for(var field in operand.vars) {
    scope['$'+field] = expr.evalExpr(operand.vars[field], scope);
  }
...
expr.ops.$map = function (operand, doc) {
  var array = expr.evalExpr(operand.input, doc);
  return array.map(function (item) {
    var scope = _.cloneDeep(doc);
    scope['$'+operand.as] = item;
    return expr.evalExpr(operand.in, scope);
  });
};
